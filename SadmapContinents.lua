...
function GetMapScriptInfo()
	...
	return {
		Name = "Sadmap Continents 3",
		Description = "A variation on LekMap Continents",
		IsAdvancedMap = false,
		IconIndex = 1,
		SortIndex = 2,
...
function GetMapInitData(worldSize)
	...
	local LandSizeXLarge = 44 + (48);
	local LandSizeYLarge = 34 + (42);
...
function ContinentsFractalWorld:GeneratePlotTypes(args)
	...
	while done == false do
		...
		iWaterThreshold = (self.continentsFrac:GetHeight(water_percent)) * (7/8);
	...
	self:ShiftPlotTypes();
		...
		local islandQty = {
			[WorldSizeTypes.WORLDSIZE_DUEL]		= 7,
			[WorldSizeTypes.WORLDSIZE_TINY]     = 9,
			[WorldSizeTypes.WORLDSIZE_SMALL]    = 12,
			[WorldSizeTypes.WORLDSIZE_STANDARD] = 17,
			[WorldSizeTypes.WORLDSIZE_LARGE]    = 22,
			[WorldSizeTypes.WORLDSIZE_HUGE]		= 26
		}
		...
		local minIslandSize = 2;
		local maxIslandSize = 6;
